...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  V3F att_dot = rot*gyro;

  float predictedPitch = pitchEst + dtIMU*att_dot.y;
  float predictedRoll = rollEst + dtIMU*att_dot.x;
  ekfState(6) = ekfState(6) + dtIMU*att_dot.z;	// yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F attitude_I = attitude.Rotate_BtoI(accel);
  predictedState(0) = curState(0) + dt*curState(3);
  predictedState(1) = curState(1) + dt*curState(4);
  predictedState(2) = curState(2) + dt*curState(5);
  predictedState(3) = curState(3) + dt*attitude_I.x;
  predictedState(4) = curState(4)+dt*attitude_I.y;
  predictedState(5) = -dt*CONST_GRAVITY + curState(5) + dt*attitude_I.z ;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0,1) = -1*sin(roll)*sin(pitch)*sin(yaw) - cos(pitch)*cos(yaw);
